...
for d, r in zip(depthes, rgbs):
    ...
    for w in range(width):
        for h in range(height):
            # if the depth pixel is a black hole,
            # then a white pixel will be generated on the same position in the mask image
            if depth[w, h] == 0.0:
                mat[w, h] = 255
    ...
    # scaling depth maps to enable using cv2.inpaint()
    scale = np.abs(depth).max()
    depth = depth.astype(np.float32) / scale  # Has to be float32, 64 not supported.

    # inpainting
    dst = cv2.inpaint(depth, mask, 1, cv2.INPAINT_NS)  # cv2.INPAINT_TELEA or cv2.INPAINT_NS
    # Back to original size and value range.
    # dst = dst[1:-1, 1:-1]
    dst = dst * scale

    # save inpainted depth map
